...
def create_tables():
	...
	data_vulnerability = [('xss', '(?<!(trim\())(\$\_(POST|REQUEST|GET)\[.*\])', 'global variables that do not start with the function trim', '1'),
			      ('xss', '(?<!(strip\_tags\())(\$\_(POST|REQUEST|GET)\[.*\])', 'global variables that do not start with the function strip_tags ', '1'),
			      ('xss', '(?<!(htmlspecialchars\())(\$\_(POST|REQUEST|GET)\[.*\])', 'global variables that do not start with the function htmlespecialchars', '1'),
			      ('xss', '(?<!(htmlentities\())(\$\_(POST|REQUEST|GET)\[.*\])', 'global variables that do not start with the function htmlentities', '1'),
                              ('xss', '>.*?<\s*\/?[\w\s]+>', 'Unquoted HTML breaking with closing tag', '2'),
			      ('xss', '[\"\'].*?>', 'HTML breaking', '2'),
		     	      ('xss', '<base\b.+?\bhref\b.+?>', 'Common JavaScript injection points (forms)', '3'),
			      ('xss', '\bon\w+\s*=', 'HTML event handler', '3'),
			      ('lfi&rfi', '(?<!(include\s))(\$\_(POST|REQUEST|GET)\[.*\])', 'Critical PHP function include', '4'),
			      ...
			      ('lfi&rfi', '\.(ht(access|passwd|group))|(apache|httpd)\d?\.conf', 'Directory traversal', '5'),
			      ('lfi&rfi', '\/etc\/[.\/]*(passwd|shadow|master\.passwd)', 'Directory traversal', '5'),
                              ('lfi&rfi', '%\w+%', 'Windows environment variable pattern', '6'),
                              ('lfi&rfi', '(?<!\w)(boot\.ini|global\.asa|sam)\b', 'Common Windows files', '6'),
                              ('lfi&rfi', '\bfile_(get|put)_contents\b.*?\(.+?\)', 'Critical PHP function file_get_contents/file_put_contents', '6'),
		              ('lfi&rfi', '\breadfile\b.*?\(.+?\)', 'Critical PHP function readfile', '4'),
                              ('lfi&rfi', '\binclude(_once)?\b.*?;', 'Critical PHP function include', '4'),
                              ('lfi&rfi', '\brequire(_once)?\b.*?;', 'Critical PHP function require', '4'),
			      ('lfi&rfi', '\(\)\s*\{.*?;\s*\}\s*;', 'Shellshock (CVE-2014-6271)', '6'),
     			      ('lfi&rfi', '(\b(do|while|for)\b.*?\([^)]*\).*?\{)|(\}.*?\b(do|while|for)\b.*?\([^)]*\))', 'C-style loops', '6'),
                              ('lfi&rfi', '[=(].+?\?.+?\:', 'C-style ternary operator', '6'),
                              ('lfi&rfi', '\\u00[a-f0-9]{2}', 'Octal entity', '7'),
                              ('lfi&rfi', '\\x0*[a-f0-9]{2}', 'Hex entity', '7'),
                              ('lfi&rfi', '<\?(?!xml\s)', 'PHP opening tag', '9'),
                              ('lfi&rfi', '\/\/input', 'PHP input stream', '8'),
                              ('lfi&rfi', '\bcall_user_func\b.*?\(.+?\)', 'Critical PHP function call_user_func', '10'),
                              ('lfi&rfi', '\bcreate_function\b.*?\(.+?\)', 'Critical PHP function call_user_func', '10'),
                              ('lfi&rfi', '\beval\b.*?(\(.+?\)|\{.+?\})', 'Critical function eval', '10'),
                              ('lfi&rfi', '\bexec\b.*?\(.+?\)', 'Critical function exec', '10'),
                              ('lfi&rfi', '\bmove_uploaded_file\b.*?\(.+?\)', 'Critical function move_uploaded_file', '11'),
                              ('lfi&rfi', '\bpassthru\b.*?\(.+?\)', 'Critical PHP function passthru', '8'),
                              ('lfi&rfi', '\bp(roc_)?open\b.*?\(.+?\)', 'Critical PHP function popen/proc_open', '10'),
                              ('lfi&rfi', 'shell_exec.*?\(.+?\)', 'critical PHP function shell_Exec', '10'),
                              ('lfi&rfi', 'system.*?\(.+?\)', 'Critical PHP function system', '10'),
                              ('lfi&rfi', '\bpreg_(replace|match)\b.*?\(.+?\)', 'Critical PHP function preg_match/preg_replace', '12'),
			      ('lfi&rfi', '\{\s*\$\s*\{.+?\}\s*\}', 'PHP complex curly syntax', '13'),
			      ('lfi&rfi', '\bexec\b.+?\bxp_cmdshell\b', 'MSSQL code execution xp_cmdshell', '10'),
       			      ('ci', 'system\s\(', 'function system', '14'),
                              ('ci', 'exec\(', 'command exec', '14'),
