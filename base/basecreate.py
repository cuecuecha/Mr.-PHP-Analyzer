...
def create_tables():
	...
	data_name_vul = [('sqli',' '),('xss', ' '),('sc', ' '),('send', ' '),('lfi&rfi', ' '),('pt', ' '),('ci', ' '),('ifc', ' '),('fxss',' '),('fci',' '),('iifc', ' '),('scok', ' '),('fsqli',' '),('fersqli',' ')]
	data_vulnerability = [
			#('1', 'find_in_set.*?\(.+?,.+?\)', 'Common MySQL function find_in_set', '16'),
			('13', '(\$.*)=(\s)*\"(SELECT|select|update|UDATE|insert|INSERT|DROP|drop)\s.*', 'MySQL information disclosure mysql.user', '16'),
			('13', '(\s)*(\$[a-zA-Z0-9]*)(\s)?\.=(.*\$\_(POST|GET)(\[.*\]))', 'Common SQL command union select', '16'),
			('13', '(\$.*)=(\s)*\"(SELECT|select|update|UDATE|insert|INSERT|DROP|drop)\s(.*\$\_(POST|GET)(\[.*\]))', 'Common SQL command union select', '16'),
			('13', '.*\"\s*\$\w*.*', 'Common SQL command update', '16'),
			('13', '.*?(\$[a-zA-Z0-9]*)\s*=\s*(.*?\$\_(POST|GET)(\[.*\]))', 'Common SQL command union select', '16'),
			('13', '(.*?((preg|ereg)\_(match|replace))\(.*?\$.*?\/(m|e).*\))', 'Common SQL command drop', '16'),
			('13', '(.*)((is\_(array|int|integer|long|numeric|string)|ctype\_(cntrl|graph|punct|xdigit|space|digit)|settype|sprintf)\(\$\_.*\)).*', 'MySQL file disclosure load_file', '16'),
			('13', '(.*?((preg|ereg)\_(match|replace))\(.*\))', 'MySQL file disclosure load_data', '16'),
			('14', '(\/\s\/)|(\/\\\\s\+\/)|(\/\^.*)|(\/.*\$\/)', 'MySQL file disclosure into outfile', '16'),
			('1', '(SELECT|select|update|UDATE|insert|INSERT|DROP|drop)\s.*', 'MySQL function concat', '16'),
			('14', '(\/\^.*\$\/)', 'the best filter', '16'),
			...
			('100', '[=(].+?\?.+?\:', 'C-style ternary operator', '6'),
			('100', '\\u00[a-f0-9]{2}', 'Octal entity', '7'),
			('100', '\/\/input', 'PHP input stream', '8'),
			('5', '^call_user_func.*?\(.+?\)', 'Critical PHP function call_user_func', '10'),
			('5', 'create_function.*?\(.+?\)', 'Critical PHP function create_function', '10'),
			('5', 'eval\s.*?(\(.+?\)|\{.+?\})', 'Critical function eval', '10'),
			('5', '^(@?exec)\(.*\)', 'Critical function exec', '10'),
			('5', 'move_uploaded_file.*?\(.+?\)', 'Critical function move_uploaded_file', '11'), #duda
			('5', 'p(roc_)?open.*?\(.+?\)', 'Critical PHP function popen/proc_open', '10'), #duda 
